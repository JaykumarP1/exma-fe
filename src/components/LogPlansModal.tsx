...
export const LogPlansModal: React.FC<LogPlansModalProps> = ({
  isOpen,
  onClose,
  logItem
}) => {
    ...
    <div style={{
      ...
      <div className="glass-panel animate-fade-in" style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <Clock size={12} /> Time Window: <strong style={{ color: '#e2e8f0' }}>{logItem.interval_formatted}</strong>
              </p>
            </div>
          </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.82rem', color: '#f8fafc', fontWeight: 700 }}>
            <Layers size={16} style={{ color: '#38bdf8' }} />
            <span>{plans.length} Execution Plans Recorded</span>
          </div>

          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <span style={{ fontSize: '0.78rem', fontWeight: 700, padding: '0.2rem 0.5rem', borderRadius: '4px', background: 'rgba(56, 189, 248, 0.15)', color: '#38bdf8' }}>
              <Zap size={12} style={{ display: 'inline', marginRight: '0.25rem' }} />
              {logItem.formatted_delta} Tokens
            </span>
            <span style={{ fontSize: '0.75rem', color: 'var(--text-dim)' }}>
              {logItem.fetched_at_formatted}
            </span>
          </div>
        </div>
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <span style={{ fontSize: '0.7rem', fontWeight: 700, padding: '0.15rem 0.45rem', borderRadius: '4px', background: 'rgba(99, 102, 241, 0.2)', color: '#818cf8' }}>
                      Plan #{idx + 1}
                    </span>
                  ...
                  <span style={{
                    ...
                    borderRadius: '4px',
                    background: plan.status === 'completed' || plan.status === 'verified' ? 'rgba(16, 185, 129, 0.2)' : 'rgba(245, 158, 11, 0.2)',
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem', paddingLeft: '0.5rem' }}>
                  {plan.steps.map((step, sIdx) => (
                    <div key={sIdx} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', fontSize: '0.82rem', color: '#cbd5e1' }}>
